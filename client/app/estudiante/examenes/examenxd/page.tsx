...
const Page = () => {
    ...
    const startTimer = () => {
        timerRef.current = window.setInterval(() => {
            if (timeLeft === 0) {
                // Si el tiempo restante es 0, reiniciar el temporizador y pasar a la siguiente pregunta
                clearInterval(timerRef.current);
                nextQuestion();
                setTimeLeft(tiempoPregunta);
            } else {
                // Si el tiempo restante no es 0, decrementarlo
    ...
    const nextQuestion = () => {
        if (activeIndex < preguntas.length) {
            startTimer();
                ...
                content: (
                    <div className="flex flex-column align-items-left" style={{ flex: '1' }}>
                        ...
                        <div className="font-medium text-lg my-3 text-900">
                            Correctas: {respuestasEstudiante.filter((respuesta: boolean) => respuesta === true).length} / {respuestasEstudiante.length} <br />
                            En blanco: {preguntas.length - respuestasEstudiante.length}
                        </div>
