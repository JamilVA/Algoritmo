...
const fetchWithTimeout = (url: string, options: RequestInit, timeout: number = 10000): Promise<Response> => {
    return Promise.race([fetch(url, options), new Promise<Response>((_, reject) => setTimeout(() => reject(new Error('Tiempo de espera agotado, por favor intente de nuevo')), timeout))]);
};
